 ...
 * to a HTTP response.
 */
server.errorHandler(() => import('#exceptions/http/handler'))
 ...
 */
server.use([
  () => import('#middleware/http/force_json_response_middleware'),
  () => import('#middleware/http/container_bindings_middleware'),
...
router.use([
  ...
  () => import('#middleware/http/silent_auth_middleware'),
  () => import('#middleware/http/initialize_bouncer_middleware'),
])
